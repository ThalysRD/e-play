...
async function patchUser(payload) {
    ...
    if (!res.ok) throw new Error(data?.message || data?.error || "Falha na atualização.");
    return data;
}

function Message({ kind, children }) {
    if (!children) return null;
    const cls = kind === "error" ? styles.errorMessage || styles.errorText : styles.successMessage || styles.successText;
    return <p className={cls} role="alert">{children}</p>;
}

function SubmitButton({ busy, children }) {
    return (
        <button type="submit" className={`${styles.button} ${styles.buttonSave}`} disabled={busy}>
        ...
        <div className={styles.pageContainer}>
            ...
            <main className={styles.body}>
                <div className={styles.formInfosPessoais}>
                    <div className={styles.formExplain}><p className={styles.title}>Informações pessoais</p></div>
                    <PersonalInfosForm user={user} onOpenModal={() => setIsModalOpen(true)} />
                </div>

                <div className={styles.formSenha}>
                    <div className={styles.formExplain}><p className={styles.title}>Redefinir senha</p></div>
